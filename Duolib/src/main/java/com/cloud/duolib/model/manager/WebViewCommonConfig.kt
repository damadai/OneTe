...
class WebViewCommonConfig(private val progressBar: ProgressBar?) {
    ...
    fun initWebView(vg: ViewGroup, url: String, fra: Fragment) {
        try {
            vg.removeAllViews()
            mAgentWeb = AgentWeb.with(fra)
                .setAgentWebParent(
                    vg,
                    -1,
    ...
    fun initWebView(vg: ViewGroup, url: String, act: Activity) {
        try {
            vg.removeAllViews()
    ...
    private fun getInitAgentWeb() {
        if (CloudBuilder.getShowLog()) {
            AgentWebConfig.debug()
        }
        if (mAgentWeb != null) {
            mAgentWeb!!.webCreator.webView.setLayerType(View.LAYER_TYPE_NONE, null)
            mAgentWeb!!.webCreator.webView.settings.mediaPlaybackRequiresUserGesture = false
    ...
    fun refreshUrl(vg: ViewGroup, url: String, fra: Fragment) {
        if (mAgentWeb == null) {
            initWebView(vg, url, fra)
        } else {
            mAgentWeb?.urlLoader?.loadUrl(url)
        }
    }

    fun refreshUrl(vg: ViewGroup, url: String, fra: Activity) {
        if (mAgentWeb == null) {
            initWebView(vg, url, fra)
        } else {
            mAgentWeb?.urlLoader?.loadUrl(url)
