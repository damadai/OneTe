...
class SmoothCheckBox : View, Checkable {
    ...
    public fun setColor(tik: Int?, check: Int?, unCheck: Int?, floor: Int?, unFloor: Int?) {
        if (tik != null) {
            mTickColor = tik
        }
        if (check != null) {
        ...
        if (unCheck != null) {
            mUnCheckedColor = unCheck
        }
        if (floor != null) {
            mFloorColor = floor
        }
        if (unFloor != null) {
            mFloorUnCheckedColor = unFloor
        }
    }
    ...
    private fun init(attrs: AttributeSet?) {
        ...
        mTickColor = ta.getColor(R.styleable.SmoothCheckBox_color_tick,  Color.WHITE)
        mFloorColor = ta.getColor(R.styleable.SmoothCheckBox_color_unchecked_stroke, Color.BLACK)
        mCheckedColor = ta.getColor(R.styleable.SmoothCheckBox_color_checked, Color.BLUE)
        mUnCheckedColor = ta.getColor(R.styleable.SmoothCheckBox_color_unchecked, Color.GRAY)
        mStrokeWidth = ta.getDimensionPixelSize(R.styleable.SmoothCheckBox_stroke_width,
            DisplayUtil.dip2px(context, 0f))
        ta.recycle()
    ...
    private fun measureSize(measureSpec: Int): Int {
        val defSize = DisplayUtil.dip2px(context, DEF_DRAW_SIZE.toFloat())
    ...
    private fun startUnCheckedAnimation() {
        ...
        animator.addUpdateListener { animation ->
            mScaleVal = animation.animatedValue as Float
            mFloorColor = getGradientColor(mCheckedColor, mTickColor, mScaleVal)
            postInvalidate()
        }
        animator.start()
